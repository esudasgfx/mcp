...
public sealed class ConfigStoreService : IConfigStoreService
{
    ...
    private IEnumerable<ConfigSetting> BuildDefaultSettings()
    {
        ...
        yield return Build("P6", "BaseUrl", _configuration["P6:BaseUrl"] ?? "https://primavera.iges.in:18206/p6ws/restapi", "Primavera P6 EPPM REST base URL.");
        yield return Build("P6", "AuthMode", _configuration["P6:AuthMode"] ?? "basic", "P6 REST auth mode: basic, bearer, auth_token, or none.");
        ...
        yield return Build("P6", "WbsEndpoint", _configuration["P6:WbsEndpoint"] ?? "wbs", "Relative P6 REST endpoint used to fetch WBS records.");
        yield return Build("P6", "RelationshipEndpoint", _configuration["P6:RelationshipEndpoint"] ?? "relationship", "Relative P6 REST endpoint used to fetch schedule logic relationships.");
        yield return Build("P6", "ResourceAssignmentEndpoint", _configuration["P6:ResourceAssignmentEndpoint"] ?? "resourceAssignment", "Relative P6 REST endpoint used to fetch resource assignments.");
        ...
        yield return Build("P6", "ProjectFilterTemplate", _configuration["P6:ProjectFilterTemplate"] ?? "Id:eq:{project_code}", "P6 project Filter template. {project_code} is replaced at runtime.");
        yield return Build("P6", "ActivityFilterTemplate", _configuration["P6:ActivityFilterTemplate"] ?? "ProjectId:eq:{project_code}", "P6 activity Filter template. {project_code} is replaced at runtime.");
        yield return Build("P6", "WbsFilterTemplate", _configuration["P6:WbsFilterTemplate"] ?? "ProjectId:eq:{project_code}", "P6 WBS Filter template. {project_code} is replaced at runtime.");
        yield return Build("P6", "RelationshipFilterTemplate", _configuration["P6:RelationshipFilterTemplate"] ?? "PredecessorProjectId:eq:{project_code}:or:SuccessorProjectId:eq:{project_code}", "P6 relationship Filter template. {project_code} is replaced at runtime.");
        yield return Build("P6", "ResourceAssignmentFilterTemplate", _configuration["P6:ResourceAssignmentFilterTemplate"] ?? "ProjectId:eq:{project_code}", "P6 resource assignment Filter template. {project_code} is replaced at runtime.");
        yield return Build("P6", "ProjectFields", _configuration["P6:ProjectFields"] ?? "ObjectId,Id,Name,Status,StartDate,FinishDate,PlannedStartDate,ScheduledFinishDate,DataDate,SummaryTotalFloat,SummarySchedulePercentComplete", "P6 project fields requested from the REST API.");
        yield return Build("P6", "ActivityFields", _configuration["P6:ActivityFields"] ?? "ObjectId,Id,Name,ProjectId,ProjectObjectId,WBSObjectId,Status,StatusCode,StartDate,FinishDate,PlannedStartDate,PlannedFinishDate,RemainingDuration,DurationPercentComplete,PercentComplete,TotalFloat,FreeFloat,IsCritical", "P6 activity fields requested from the REST API.");
        yield return Build("P6", "WbsFields", _configuration["P6:WbsFields"] ?? "ObjectId,Name,ProjectId,ProjectObjectId,ParentObjectId,Status,StartDate,FinishDate,SummaryTotalFloat,SummarySchedulePercentComplete", "P6 WBS fields requested from the REST API.");
        yield return Build("P6", "RelationshipFields", _configuration["P6:RelationshipFields"] ?? "ObjectId,PredecessorActivityId,PredecessorActivityName,PredecessorProjectId,SuccessorActivityId,SuccessorActivityName,SuccessorProjectId,Type,Lag", "P6 relationship fields requested from the REST API.");
        yield return Build("P6", "ResourceAssignmentFields", _configuration["P6:ResourceAssignmentFields"] ?? "ObjectId,ActivityId,ActivityName,ProjectId,ResourceId,ResourceName,RoleId,RoleName,StartDate,FinishDate,RemainingDuration,PercentComplete,StatusCode", "P6 resource assignment fields requested from the REST API.");
        ...
        yield return Build("P6", "AuthToken", "P6_AUTH_TOKEN", "Environment variable containing a P6 AuthToken header value.", isSecret: true, secretReference: "P6_AUTH_TOKEN");
        yield return Build("P6", "TokenHeader", _configuration["P6:TokenHeader"] ?? "AuthToken", "Header name used when P6 AuthMode is auth_token.");
